...
def build_systematic_re() -> re.Pattern:
    ...
    if not patterns:
        ...
        clean_parts = []
        for p in patterns:
            # Odpagesit inline (?i) z begin (TSV patterns začínají (?i)^\s*...)
            clean = re.sub(r"^\(\?[iI]\)", "", p.strip())
            clean = re.sub(r"^\(\?[iI]\)", "", clean.strip())  # druhý pruchod pro jistotu
            try:
...
def recompute_all_term_matches(progress_cb=None) -> int:
    """Recalculates terms for ALL approved candidates (rescue/batch operation)."""
    con = db()
...
def tab_library():
    ...
    with st.expander(t("upload_new"), expanded=True):
        ...
        if uploaded_files and do_up:
            ...
            overall = st.progress(0, tt(f"Zpracovavam 0/{n_files} fileu…", f"Processing 0/{n_files} files…"))
            results = []
...
def tab_settings():
    ...
    with stab1:
        ...
        if sv1.button("💾 Save displayed changes", type="primary",
            ...
            st.success(tt(f"✅ Saved {len(new_rows)} labels ", f"✅ Saved {len(new_rows)} labels ") +
                       tt(f"(celkem {len(full)} v fileu).", f"({len(full)} total in file)."))
            st.rerun()
    ...
    with stab1c:
        st.caption(
            "Terms are used in tab „Morpho/Strat.“ to filter taxa according to morphological characters and stratigraphic units. Matches are calculated automatically when a record is approved.")
        ...
        with mt1:
            ...
            mm1.metric("Terms", len(morph_df))
            mm2.metric("Categories", morph_df["category"].nunique() if "category" in morph_df.columns else 0)
            morph_src = ("✅ " + str(MORPHOLOGY_FILE)) if MORPHOLOGY_FILE.exists() else "⚠️ No file"
            st.caption(morph_src)
            new_morph = st.file_uploader(
                "Upload new list of morphological terms (TSV)",
            ...
            if new_morph and st.button(t("save_morpho_btn")):
                ...
                st.success(tt(f"Saved ({new_morph.size} B).", f"Saved ({new_morph.size} B)."))
                st.rerun()
        ...
        with mt2:
            ...
            sm1.metric("Terms", len(strat_df))
            sm2.metric("Categories", strat_df["category"].nunique() if "category" in strat_df.columns else 0)
            strat_src = ("✅ " + str(STRATIGRAPHY_FILE)) if STRATIGRAPHY_FILE.exists() else "⚠️ No file"
            st.caption(strat_src)
            new_strat = st.file_uploader(
                "Upload new list of stratigraphical terms (TSV)",
            ...
            if new_strat and st.button(t("save_strat_btn")):
                ...
                st.success(tt(f"Saved ({new_strat.size} B).", f"Saved ({new_strat.size} B)."))
                st.rerun()
            with st.expander(t("terms_preview")):
        ...
        with mt3:
            ...
            st.caption(
                "Headings for the systematic/taxonomic section of the article in 12 languages. They determine where the `systematic` zone begins—blocks within it receive a +0.15 bonus to their score; blocks outside it receive a penalty. File: `systematic_sections.tsv`  |  columns: `enabled, language, heading, priority, weight, pattern, note`")
            sys_df = load_systematic_sections()
            sy1, sy2, sy3 = st.columns(3)
            sy1.metric("Headings", len(sys_df))
            sy2.metric("Languages", sys_df["language"].nunique() if "language" in sys_df.columns else 0)
            sy3.metric("Core", int((sys_df["priority"] == "core").sum()) if "priority" in sys_df.columns else 0)
            sys_src = ("✅ " + str(SYSTEMATIC_SECTIONS_FILE)) if SYSTEMATIC_SECTIONS_FILE.exists() else "⚠️ No file (použit fallback)"
            st.caption(sys_src)
            with st.expander(t("sys_sections_edit"), expanded=False):
                ...
                if st.button(t("save_sys_sections_btn"), key="sys_sec_save", type="primary"):
                    ...
                    st.success(tt(f"Saved {len(_sys_edit)} rows. Changes will take effect on next indexing.", f"Saved {len(_sys_edit)} rows. Changes will take effect at next indexing."))
                    st.rerun()
            new_sys = st.file_uploader(
                "Upload TSV (systematic_sections.tsv)",
            ...
            if new_sys and st.button(t("upload_file_btn"), key="sys_sec_file_save"):
                ...
                reload_systematic_sections()
                st.success(tt(f"Saved ({new_sys.size} B).", f"Saved ({new_sys.size} B).")); st.rerun()

        st.divider()
        if st.button(tt("🔄 Recalculate terms for all approved records", "🔄 Recompute terms for all approved records"),
            ...
            st.toast(tt(f"⏳ Recalculating {len(_s_ids)} records — progress uvidite nahore.", f"⏳ Recomputing {len(_s_ids)} records — progress shown above."), icon="⏳")
            st.rerun()
